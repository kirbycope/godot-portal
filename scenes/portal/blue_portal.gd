...
func _process(_delta: float) -> void:
	...
	if camera != null:
		...
		# Move the 🎥 camera from behind the 🔴 Red Portal to behind the 🔵 Blue Portal,
		# so that the 🔵 Blue Portal's SubViewport now displays what it would look like looking out of the 🔴 Red Portal.
...
func _on_body_entered(body: Node3D) -> void:
	...
	if body is CharacterBody3D:
		...
		if camera != null:
			...
			body.global_transform = $"../RedPortal/ExitPoint".global_transform
